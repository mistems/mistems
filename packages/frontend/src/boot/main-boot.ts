...
export async function mainBoot() {
	...
	if ($i) {
		...
		miRegistryItem.get('channelsLastReadedAt').then(channelsLastReadedAt => {
			miLocalStorage.setItemAsJson('channelsLastReadedAt', channelsLastReadedAt);
		}).catch(() => {
			// 取得に失敗しても既存のローカル値で動作できるので無視する
		});
