...
<section v-if="!hasChildSection" v-panel style="border-radius: 6px; border-bottom: 0.5px solid var(--MI_THEME-divider);">
	...
	<div v-if="!shown" class="body" style="display: flex; align-items: center;">
		...
		<i v-if="emojis.length > 4" class="toggle ti-fw" style="font-size: 14px;" :class="shown ? 'ti ti-minus' : 'ti ti-plus'" @click="shown = true"></i>
		<span v-if="emojis.length > 4" style="font-size: 14px;" @click="shown = true">({{ (emojis.length - 4) }})</span>
	</div>
</section>
