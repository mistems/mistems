	...
	<div :class="$style.container">
		...
		<div v-else style="display: contents;">
			...
			<div v-if="targetDate !== null && canLoadNewer && !isTimeshiftMode" :class="$style.loadMore">
				...
				<div :class="$style.loadMoreRow">
					<MkButton primary @click="startTimeshift" style="flex: 1;">
						<i class="ti ti-player-play"></i> Timeshift
					</MkButton>
					<MkButton primary @click="startTimefusion" style="flex: 1;">
			...
			<!-- タイムライン表示 -->
			<div v-if="paginator" :class="$style.timeline" ref="scrollContainer">
...
import { ref, computed, markRaw, provide, watch, onUnmounted, nextTick, type Ref } from 'vue';
...
const timelineSource = ref<'home' | 'local' | 'social' | 'global'>(
	props.column.timemachineTl ?? 'social'
);
const dateInput = ref<string>(
	props.column.timemachineDate ??
	`${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`
);
const timeInput = ref<string>(
	props.column.timemachineTime ??
...
const isValidDateTime = computed(() => {
	return typeof targetDate.value === 'number' && targetDate.value < now.getTime();
});
...
async function loadNewer() {
	if (!paginator.value || isLoadingNewer.value || !scrollContainer.value) return;
