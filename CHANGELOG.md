...
### Server
- Enhance: 冗長なデータベースインデックスを削除し、noteテーブルのNULLが支配的なカラムのインデックスを部分インデックスに置き換えることでストレージ使用量と書き込み負荷を軽減
  - 大規模インスタンスでは環境変数 `MISSKEY_MIGRATION_CREATE_INDEX_CONCURRENTLY=1` を設定してマイグレーションを実行することを推奨します (未設定の場合、インデックス再構築中はnoteテーブルへの書き込みがブロックされます)
- Enhance: グループ化チャート (ユーザー毎・インスタンス毎) のテーブルに (group, date) インデックスを追加し、最終更新が古いグループのチャート取得が極端に遅くなる問題を改善
- Enhance: ユーザー検索 (メンション補完) の前方一致に text_pattern_ops インデックスを追加し、C 以外の照合順序で全走査になっていた問題を改善
