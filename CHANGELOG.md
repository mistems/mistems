...
### Client
- Feat: 投稿フォームで、センシティブワード(公開範囲がホームになる語)が含まれている場合に警告を表示し、プレビューで該当箇所を黄色背景でハイライトするように
- Feat: 投稿フォームの公開範囲ピッカーからお気に入りのチャンネルを宛先に選べるように
- Feat: チャンネルページでローカルタイムラインへの投稿ボタンを表示するかを設定できるように
- Feat: 投稿フォームにCWと本文を入れ替えるボタンを追加
- Feat: ショートカットキーを拡充 (h: ヘルプ表示, p: 通常の投稿, n: チャンネルページではチャンネルへの投稿)
- Enhance: チャンネルの既読状態をアカウントに同期するように (端末間で共有されます)
- Enhance: スマートフォンで投稿フォームより上の背景をタップしても閉じないように
- Enhance: 長文ペースト時のファイル添付確認を改善 (確認ボタンの文言を明確化、しきい値を文字数上限に連動)
...
### Server
- Feat: `meta` のレスポンスに `sensitiveWords` を追加 (クライアントの投稿フォームでセンシティブワードを警告表示するため)
