...
### Client
- Enhance: ノート検索で検索範囲に「チャンネル指定」を追加 (お気に入りチャンネルから選択)
- Enhance: ノート検索がタイムアウト (15秒) した場合に「直近1か月 / 直近3か月で再検索」を提案するバナーを表示
- Enhance: ノート検索の検索ボックスに対応している検索構文 (AND / OR / -除外 / フレーズ) のヘルプを表示
...
- Enhance: ノート検索の CW 込み検索 (PGroonga) を式インデックスと一致する連結式に固定し、インデックスが確実に使われるように (本番実測 23.7s → 1.2s)
